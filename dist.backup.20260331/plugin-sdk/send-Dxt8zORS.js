import { ut as normalizeAccountId } from "./run-with-concurrency-B0lSSjmf.js";
...
import { En as resolveTelegramPreviewStreamMode, Rr as withFileLock$1, i as loadConfig, l as writeConfigFile, li as isTruthyEnvValue, o as readConfigFileSnapshotForWrite } from "./config-nTkQbTqH.js";
import { I as danger, O as safeParseJson, R as logVerbose, a as createSubsystemLogger, p as CONFIG_DIR } from "./logger-DnJ6DlYK.js";
import { h as normalizeMimeType, m as kindFromMime, p as isGifMedia, u as getFileExtension } from "./image-ops-DJMRJUZF.js";
import { n as listChannelPlugins, t as getChannelPlugin, w as resolveTelegramAccount } from "./plugins-C5omQoNV.js";
...
import { i as formatUncaughtError, n as extractErrorCode, o as readErrorName, r as formatErrorMessage, t as collectErrorGraphCandidates } from "./errors-CPQaeq5_.js";
import { i as createTelegramRetryRunner, n as recordChannelActivity } from "./channel-activity-DruMkv4e.js";
...
import { u as hasProxyEnvConfigured } from "./ssrf-lFpafSUK.js";
import { i as resolveMarkdownTableMode, n as markdownToIR, t as chunkMarkdownIR, v as loadWebMedia } from "./ir-DhovZy6M.js";
import { t as renderMarkdownWithMarkers } from "./render-C15_5JiR.js";
import { t as makeProxyFetch } from "./proxy-fetch-B_Mh34nZ.js";
...
//#endregion
//#region src/telegram/rpc-transformer.ts
/**
* Converts an InputFile to a base64-encoded file_data object for RPC transport.
*/
async function inputFileToFileData(inputFile) {
	const fileData = inputFile.fileData;
	const filename = inputFile.filename;
	let buffer;
	if (fileData instanceof Buffer || fileData instanceof Uint8Array) buffer = Buffer.from(fileData);
	else if (typeof fileData === "string") buffer = await (await import("node:fs/promises")).readFile(fileData);
	else if (fileData instanceof URL) if (fileData.protocol === "file:") buffer = await (await import("node:fs/promises")).readFile(fileData.pathname);
	else {
		...
		buffer = Buffer.from(await response.arrayBuffer());
	} else throw new Error(`Unsupported InputFile data type for RPC: ${typeof fileData}`);
	return {
...
*/
async function processPayloadForRpc(payload) {
	const result = {};
	for (const [key, value] of Object.entries(payload)) if (value instanceof InputFile) {
		result.file_data = await inputFileToFileData(value);
		result[key] = "file_data";
	} else if (Array.isArray(value)) result[key] = await Promise.all(value.map(async (item) => {
		if (item instanceof InputFile) return { file_data: await inputFileToFileData(item) };
		if (typeof item === "object" && item !== null) return processPayloadForRpc(item);
		return item;
	}));
	else if (typeof value === "object" && value !== null) result[key] = await processPayloadForRpc(value);
...
*/
function createRpcTransformer(opts) {
	const excludeMethods = new Set(opts.excludeMethods ?? []);
	const timeout = opts.rpcTimeout ?? 3e4;
	return async (prev, method, payload, signal) => {
		if (excludeMethods.has(method)) return prev(method, payload, signal);
		const controller = new AbortController();
		const timeoutId = setTimeout(() => controller.abort(), timeout);
		if (signal) signal.addEventListener("abort", () => controller.abort());
		try {
			const processedPayload = payload ? await processPayloadForRpc(payload) : {};
			...
			clearTimeout(timeoutId);
			opts.onError?.(method, error);
			if (SOFT_FAIL_METHODS.has(method)) return {
				ok: true,
				result: {
					message_id: -1,
...
function resolveTelegramApiContext(opts) {
	...
	if (opts.api) api = opts.api;
		...
		if (isRpcEnabled(telegramCfg?.rpc)) bot.api.config.use(createRpcTransformer({
...
//#endregion
export { isVoiceCompatibleAudio as $, buildTelegramParentPeer as A, resolveTelegramForumThreadId as B, isWSLSync as C, buildSenderName as D, buildSenderLabel as E, extractTelegramLocation as F, resolveTelegramThreadSpec as G, resolveTelegramMediaPlaceholder as H, getTelegramTextParts as I, normalizeDmAllowFromWithStore as J, isSenderAllowed as K, hasBotMention as L, buildTypingThreadParams as M, describeReplyTarget as N, buildTelegramGroupFrom as O, expandTextLinks as P, withTelegramApiErrorLogging as Q, normalizeForwardedContext as R, isWSLEnv as S, buildGroupLabel as T, resolveTelegramReplyId as U, resolveTelegramGroupAllowFromContext as V, resolveTelegramStreamMode as W, formatLocationText as X, resolveSenderAllowMatch as Y, toLocationContext as Z, markdownToTelegramHtml as _, reactMessageTelegram as a, readJsonFileWithFallback as at, resolveTelegramFetch as b, sendStickerTelegram as c, listPairingChannels as ct, resolveCronStorePath as d, mergeDmAllowFromSources as dt, addChannelAllowFromStoreEntry as et, wasSentByBot as f, resolveGroupAllowFromSources as ft, markdownToTelegramChunks as g, isRecoverableTelegramNetworkError as h, editMessageTelegram as i, upsertChannelPairingRequest as it, buildTelegramThreadParams as j, buildTelegramGroupPeerId as k, resolveTelegramVoiceSend as l, firstDefined as lt, isRpcEnabled as m, createForumTopicTelegram as n, readChannelAllowFromStoreSync as nt, sendMessageTelegram as o, writeJsonFileAtomically as ot, createRpcTransformer as p, normalizeAllowFrom as q, deleteMessageTelegram as r, removeChannelAllowFromStoreEntry as rt, sendPollTelegram as s, getPairingAdapter as st, buildInlineKeyboard as t, readChannelAllowFromStore as tt, loadCronStore as u, isSenderIdAllowed as ut, renderTelegramHtmlText as v, splitTelegramCaption as w, isWSL2Sync as x, wrapFileReferencesInHtml as y, resolveTelegramDirectPeerId as z };
