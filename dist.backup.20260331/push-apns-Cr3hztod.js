import { r as isVerbose } from "./globals-Bv4ZcVWM.js";
import { g as resolveStateDir } from "./paths-BfR2LXbA.js";
import { s as shouldLogSubsystemToConsole, t as createSubsystemLogger } from "./subsystem-Cf9yS0UI.js";
import { E as parseAgentSessionKey } from "./session-key-51LnISpq.js";
import { i as sniffMimeFromBase64 } from "./compact-DYAmGi3u.js";
import { n as redactSensitiveText, t as getDefaultRedactPatterns } from "./redact-SKlSb-Ph.js";
...
async function parseMessageWithAttachments(message, attachments, opts) {
	...
	for (const [idx, att] of attachments.entries()) {
		...
		if (isImageMime(sniffedMime) || !sniffedMime && isImageMime(providedMime)) {
			...
			if (opts?.workspaceDir) try {
				const uploadsDir = nodePath.join(opts.workspaceDir, "uploads");
				fs$2.mkdirSync(uploadsDir, { recursive: true });
				...
				fs$2.writeFileSync(filePath, Buffer.from(b64, "base64"));
		...
		if (isTextMime(providedMime) || isTextMime(sniffedMime)) {
			try {
				const decoded = Buffer.from(b64, "base64").toString("utf-8");
				textParts.push(`\n\n[File attached: ${label}]\n\`\`\`\n${decoded}\n\`\`\``);
			} catch {
				log?.warn(`attachment ${label}: failed to decode text content, dropping`);
			}
			continue;
		}
		if (opts?.workspaceDir) try {
		...
		else log?.warn(`attachment ${label}: unsupported type (${effectiveMime}), no workspace to save, dropping`);
	}
	return {
		message: textParts.length > 0 ? message + textParts.join("") : message,
		images
	};
}
//#endregion
