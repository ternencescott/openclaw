...
function isTextMime(mime?: string): boolean {
  ...
  return textLikeMimes.includes(mime);
}
...
export async function parseMessageWithAttachments(
  ...
  for (const [idx, att] of attachments.entries()) {
    ...
    // Handle image attachments — pass to vision AND save to workspace
    if (isImageMime(sniffedMime) || (!sniffedMime && isImageMime(providedMime))) {
      ...
      // Also save image to workspace so AI can reference/manipulate it
    ...
    // Handle text/code file attachments — decode and append to message
