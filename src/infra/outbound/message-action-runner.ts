...
import { resolveSandboxedMediaSource } from "../../agents/sandbox-paths.js";
import {
  readNumberParam,
...
const WEBCHAT_FILE_MAX_INLINE_BYTES = 5 * 1024 * 1024; // 5 MB
...
async function handleWebchatFileSend(
  input: RunMessageActionParams,
  params: Record<string, unknown>,
  rawFilePath: string,
): Promise<MessageActionRunResult> {
  // Resolve file path through sandbox (maps container paths to host paths)
  let resolvedPath = rawFilePath;
  if (input.sandboxRoot) {
    resolvedPath = await resolveSandboxedMediaSource({
      media: rawFilePath,
      sandboxRoot: input.sandboxRoot,
    });
  }
  ...
  if (stat.isDirectory()) {
    throw new Error(`Cannot send a directory: ${rawFilePath}`);
  }
  ...
  // Compute workspace-relative path for HTTP download fallback.
  // Container workspace is typically at sandboxRoot + /workspace/
  let workspacePath = fileName;
  if (input.sandboxRoot) {
    const wsRoot = nodePath.join(input.sandboxRoot, "workspace");
    if (resolvedPath.startsWith(wsRoot)) {
      workspacePath = resolvedPath.slice(wsRoot.length).replace(/^\//, "");
    }
  }

  // Read file as base64 for inline delivery (small files only)
  let base64Data: string | undefined;
  if (fileSize <= WEBCHAT_FILE_MAX_INLINE_BYTES) {
  ...
  const fileResult = {
    status: "ok",
    channel: "webchat",
    message: message || `File sent: ${fileName}`,
    file: {
      name: fileName,
      path: workspacePath,
      size: fileSize,
...
export async function runMessageAction(
  ...
  // Intercept webchat sends BEFORE normalizeMessageActionInput, which would
  // throw "Action send requires a target" since webchat has no explicit target.
  // Detect webchat either from explicit channel param or from tool context (originating channel).
  const rawChannelHint = readStringParam(params, "channel");
  const isWebchatContext =
    rawChannelHint?.trim().toLowerCase() === "webchat" ||
    (!rawChannelHint && input.toolContext?.currentChannelProvider === "webchat");
  if (isWebchatContext && action === "send") {
    const fileParam =
      readStringParam(params, "filePath", { trim: false }) ??
      readStringParam(params, "path", { trim: false }) ??
      readStringParam(params, "media", { trim: false });
    if (fileParam) {
      return handleWebchatFileSend(input, params, fileParam);
    }
    // For text-only webchat sends (no target needed), return success directly.
    // The AI's message text is already visible in the chat stream.
    ...
    return {
      ...
      to: "webchat",
      handledBy: "core",
      payload: { status: "ok", channel: "webchat", message: message || "Message delivered via chat." },
      dryRun: false,
    };
  }
